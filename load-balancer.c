...
static void parse_upstreams(void) {
    ...
    if (!env || !*env) env = "/run/sock/api1.sock,/run/sock/api2.sock";
...
static void connect_all_upstreams(void) {
    for (int i = 0; i < upstream_count; i++) {
        ...
        upstreams[i].fd = fd;
        fprintf(stderr, "lb: connected control upstream %s\n", upstreams[i].path);
    }
}
    ...
    for (;;) {
        ...
        if (handoff_client_fd(idx, cfd) < 0) {
            ...
            for (int j = 1; j < upstream_count; j++) {
                ...
                if (handoff_client_fd(alt, cfd) == 0) { ok = 1; break; }
            }
            if (!ok) {
                fprintf(stderr, "lb: failed to hand off client fd: %s\n", strerror(errno));
            }
        }
...
int main(void) {
    ...
    fprintf(stderr, "lb: fd-handoff ready on :%d with %d upstream(s)\n", port, upstream_count);
